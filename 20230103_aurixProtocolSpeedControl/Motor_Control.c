...
#define target_angular_speed            150
...
//    pid_pwm = (P * pos_error) + (Pd * pos_error_d) + (Pi * pos_error_sum);
    ...
    angular_pid_pwm = angular_speed_pid_control();
    //angular pid
    pid_pwm = angular_pid_pwm;
...
int inverse_speed_mode(void){
  float pwm = 0;
  pwm = target_angular_speed * 2.3; //5.0�� ������� (sum���� �ʰ� �ö󰡱� ������ ���� �ö󰡵���)
  return pwm;
}
...
{
    motor_pwm = generate_PWM_signal(PID());
    if(motor_pwm<0.0)
    {
