...
#define encoder1                   &MODULE_P02,0            /* Pin that is toggled by pressing the board's button   */
#define encoder2                   &MODULE_P02,1            /* LED which gets toggled in Interrupt Service Routine  */
#define PI                         3.141592
...
{
    if(IfxPort_getPinState(encoder2) == 1){
        rpm_count++;
    }
    waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, 1));

    if(IfxPort_getPinState(encoder1) == 0 && IfxPort_getPinState(encoder2) == 1){
        encoderPos++;
    }
    else if(IfxPort_getPinState(encoder1) == 1 && IfxPort_getPinState(encoder2) == 1){
        encoderPos--;
    }

    if(rpm_count==1){
        time_old = counter_value()/1000;
    }

    else if(rpm_count==150){
        time = counter_value()/1000;
        rpm = (PI/(time-time_old))*60;
        rpm_count=0;
        clear_cnt();
    }
}
...
void initPeripheralsAndERU(void)
{
    ...
    IfxPort_setPinMode(encoder1,  IfxPort_Mode_inputPullUp);              /* Initialize TRIGGER_PIN port pin  */
    IfxPort_setPinMode(encoder2,  IfxPort_Mode_inputPullUp);
    //IfxPort_setPinMode(encoder2, IfxPort_Mode_inputNoPullDevice);            /* Initialize LED port pin          */
