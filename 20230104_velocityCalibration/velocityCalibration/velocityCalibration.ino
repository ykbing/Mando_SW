...
float p = 0.8;   // 0.8  0.8
float pd = 1.2;  // 1.5  1.2
float pi = 0.2;  // 0.5  0.2
...
int pos_pid_control(void)
{
  ...
  pos_pid_pwm = (pos_pid_pwm <= -255) ? -255 : pos_pid_pwm;
   ...
   if(input_velocity != 0)
   {
    pos_pid_pwm = abs(pos_pid_pwm) > abs(input_velocity) ? input_velocity : pos_pid_pwm ;
  ...
  if (Serial.available() > 0) 
  {
    ...
    if (check_char == 's')
    {
     ...
     if (check_char == 'm')
     {
      ...
      for(int j = 0; j< 3; j++)
      {
        speed_buffer[j] = Serial.read(); 
      }

      Serial.write(speed_buffer[0]);
      Serial.write(speed_buffer[1]);
      Serial.write(speed_buffer[2]);
      
      s=String(data_buffer);
      s_p=String(speed_buffer);
      
      data = s.toInt();
      speed_data = s_p.toInt();
      
      if(motor_direction =='r') data = -data;
      if(motor_direction =='r') speed_data = -speed_data;
      ...
      input_velocity = speed_data;
      target_Pos = data * 14.5;
     }
 }
...
// Serial.print(pos_error);
 ...
 Serial.println(target_Pos/14.5);
}
