...
int PD_control(void)
{
    ...
    pd_pwm = (P * pos_error) + (Pd * pos_error_d);

    pd_pwm = (pd_pwm >=  255) ?  255 : pd_pwm;
    pd_pwm = (pd_pwm <= -255) ? -255 : pd_pwm;
...
int generate_PWM_signal(int pd_pwm)
{
    ...
    g_calculatedPwmDutyCycle = (float)pd_pwm / 255 * 100;
...
{
    motor_pwm = generate_PWM_signal(PD_control());
    if(motor_pwm < 0.0)
    {
